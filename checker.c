...
static	char	**fill_array_cmd(void)
{
	char	**list;
...
static	void	exec_arg(t_list_n **stack_a, t_list_n **stack_b, char *cmd)
{
	if (ft_lstsize_int((*stack_a)) > 1 && (!ft_strncmp("sa", cmd, 2)
			|| !ft_strncmp(cmd, "ss", 2)))
		swap_stack(*stack_a, 'Q');
	if (ft_lstsize_int((*stack_b)) > 1 && (!ft_strncmp("sb", cmd, 2)
			|| !ft_strncmp(cmd, "ss", 2)))
	...
	if (ft_lstsize_int((*stack_a)) > 1 && (!ft_strncmp("rra", cmd, 3)
			|| !ft_strncmp("rrr", cmd, 3)))
		reverse_rotate(stack_a, 'Q');
	if (ft_lstsize_int((*stack_b)) > 1 && (!ft_strncmp("rrb", cmd, 3)
			|| !ft_strncmp(cmd, "rrr", 3)))
		reverse_rotate(stack_b, 'Q');
	if (ft_lstsize_int((*stack_a)) > 1 && (!ft_strncmp("ra", cmd, 2)
			|| !ft_strncmp(cmd, "rr", 2)) && ft_strlen(cmd) < 3)
		rotate(stack_a, 'Q');
	if (ft_lstsize_int((*stack_b)) > 1 && (!ft_strncmp("rb", cmd, 2)
			|| !ft_strncmp(cmd, "rr", 2)) && ft_strlen(cmd) < 3 )
		rotate(stack_b, 'Q');
}

static void	set_arg(t_list_n **stack_a, t_list_n **stack_b)
{
	char	*line;
	int		end;
	char	**comand;
